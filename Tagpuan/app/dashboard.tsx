  ...
  useEffect(() => {
    const fetchMyBids = async () => {
      try {
        ...
        const userBids = data.flatMap((request: any) => {
          if (!request.bidders) return [];
          return request.bidders
            .filter((b: any) => b.id === user.uid && (b.status === 'accepted' || b.status === 'rejected'))
            .map((b: any) => ({
            ...
            myBids.map((bid, idx) => (
              <View key={idx} style={styles.listItem}>
                <Text style={styles.listText}>
                  {bid.status === 'accepted' ? '✅ Accepted' : '❌ Rejected'} – {bid.commodity}
                </Text>
              </View>
            ))
          )}
